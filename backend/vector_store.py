...
class VectorStore:
    ...
    def search(self, 
        ...
        try:
            ...
            # Fetch lesson links for each result
            search_results.lesson_links = self._fetch_lesson_links(search_results.metadata)
    ...
    def _fetch_lesson_links(self, metadata_list: List[Dict[str, Any]]) -> List[Optional[str]]:
        """Fetch lesson links for search results"""
        import json
        lesson_links = []
        ...
        for meta in metadata_list:
            ...
            try:
                ...
                course_results = self.course_catalog.get(
                    ids=[course_title]
                )
                
                if course_results and 'metadatas' in course_results and course_results['metadatas']:
                    course_metadata = course_results['metadatas'][0]
                ...
                print(f"Error fetching lesson link for {course_title} lesson {lesson_number}: {e}")
                lesson_links.append(None)
